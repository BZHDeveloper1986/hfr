// ==UserScript==
// @name          [HFR] Flux RSS - PJ edition
// @version       1.0.61
// @description   permet d'avoir les infos de son choix en direct
// @icon          http://reho.st/self/40f387c9f48884a57e8bbe05e108ed4bd59b72ce.png
// @downloadURL   https://gitlab.com/BZHDeveloper/HFR/raw/master/HFR_Flux_RSS_pj.user.js
// @updateURL     https://gitlab.com/BZHDeveloper/HFR/raw/master/HFR_Flux_RSS_pj.user.js
...
var cmScript = {
	...
	buildConfigWindow: function() {
		...
		if(!document.getElementById("TTBRSS_front")) {
			...
			var catListContainer = document.createElement("p");
			catListContainer.appendChild (document.createTextNode ("Liste des catégories où le script est actif : "));
			var selectCatList = document.createElement ("select");
			selectCatList.id = "TTBRSS_selectCatList";
			catListContainer.appendChild (selectCatList);
			var btnDelCat = document.createElement("input");
			btnDelCat.type = "button";
			btnDelCat.value = "Supprimer";
			btnDelCat.style.marginLeft = "10px";
			catListContainer.appendChild(btnDelCat);
			cmScript.configDiv.appendChild(catListContainer);

			var clist_str = GM_getValue ("TTBRSS_cats", "");
			if (clist_str == "")
				clist_str = "{}";
			var clist = JSON.parse (clist_str);
			for (var key in clist) {
				...
				option.setAttribute ("title", key);
				selectCatList.appendChild (option);
			}
			...
			btnDelCat.addEventListener("click", function() {
				...
				if (clist_str == "")
					clist_str = "{}";
				var clist = JSON.parse (clist_str);
				var catToDelete = selectCatList.options[selectCatList.selectedIndex].value;
				delete clist[catToDelete];
				GM_setValue ("TTBRSS_cats", JSON.stringify (clist));

				//on vide les options
				selectCatList.options.length = 0;
			...
			var catContainer = document.createElement ("p");
			catContainer.appendChild (document.createTextNode ("Ajout d'une catégorie où le script sera actif : "));
			var inputCat = document.createElement ("input");
			inputCat.type = "text";
			inputCat.style.width = "60px";
			var btnAddCat = document.createElement("input");
			btnAddCat.type = "button";
			btnAddCat.value = "Ajouter";
			btnAddCat.style.marginLeft = "10px";
			inputCat.addEventListener ('keyup', function (evt) {
				btnAddCat.disabled = !(inputCat.value == "prive" || !isNaN (inputCat.value));
			});
			btnAddCat.addEventListener ('click', function (evt) {
				var clist_str = GM_getValue ("TTBRSS_cats", "");
				if (clist_str == "")
					clist_str = "{}";
				var clist = JSON.parse (clist_str);
				clist[inputCat.value] = inputCat.value;
				GM_setValue ("TTBRSS_cats", JSON.stringify (clist));
				//on vide les options
				selectCatList.options.length = 0;
			...
			catContainer.appendChild (inputCat);
			catContainer.appendChild (btnAddCat);
			cmScript.configDiv.appendChild (catContainer);
	...
	createConfigMenu: function() {
		GM_registerMenuCommand("[HFR] Flux RSS - PJ edition -> configuration", this.showConfigWindow);
	}
};
...
function main() {
	...
	var cat = q.split ("#")[0].split ("cat=")[1].split ("&")[0];
	if (!clist.hasOwnProperty (cat))
		return;
	...
	var b0 = document.getElementsByClassName ("message")[0];
	var b1 = document.getElementsByClassName ("messCase1")[0];
	var b2 = document.getElementsByClassName ("messCase2")[0];
	var w0 = (b0 == null) ? 0 : b0.getBoundingClientRect().width;
	var w1 = (b1 == null) ? 0 : b1.getBoundingClientRect().width;
	var w2 = (b2 == null) ? 0 : b2.getBoundingClientRect().width;
